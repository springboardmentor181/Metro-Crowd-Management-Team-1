...
type ApiUser = { id: string; email: string; full_name: string; role: string; network: string; is_active: boolean };
const getUsers = async (): Promise<ApiUser[]> => [{ id: "1", email: "admin@metroflow.app", full_name: "Admin User", role: "admin", network: "All networks", is_active: true }];
const inviteUser = async (data: any) => ({ ok: true, error: "" });
const field =
