...
const STATS = [
  {
    value: `${(crowd.final_test.accuracy * 100).toFixed(1)}%`,
    label: "Crowd accuracy",
  },
  {
    value: `${crowd.final_test.macro_f1}`,
    label: "Macro-F1",
  },
  {
    value: `${(
...
const MODELS = [
  {
    ...
    algo: "Random Forest · Critical-class optimized",
    desc: "Predicts next-hour crowd level (Low / Medium / High / Critical) per station using operational, temporal and historical passenger features, with additional weight given to the rare Critical class.",
  },
  {
    icon: TrendingUp,
    name: "Demand forecaster",
    algo: "XGBoost Regression",
    desc: "Forecasts passenger demand using historical passenger volumes, occupancy, time-based features, operational signals, weather and event information.",
  },
  {
    icon: GitBranch,
      ...
      <section className="bg-[color:var(--color-brand-900)] pt-28 pb-16 text-white">
        <div className="container-mf">
          ...
          <p className="mt-4 max-w-2xl text-white/85">
            MetroFlow estimates crowds from ticketing and operational data —
            not cameras. Every prediction ships with a confidence score and is
      ...
      <section className="container-mf py-20">
        <Reveal>
          <div className="card overflow-hidden">
            ...
            >
              <source
                src="/assets/video/ai-dataviz-anim.mp4"
                type="video/mp4"
              />
            </video>
          </div>
        ...
        {/* Real trained production metrics */}
        <Reveal delay={0.1}>
          <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
            {STATS.map((s) => (
              <div key={s.label} className="card p-5 text-center">
                <div className="font-display text-3xl font-bold text-[color:var(--color-ai)]">
                  {s.value}
                </div>

                <div className="mt-1 text-xs text-[color:var(--color-ink-2)]">
                  {s.label}
                </div>
              </div>
            ))}
          ...
          <p className="mt-3 text-center text-xs text-[color:var(--color-muted)]">
            Production metrics from the Random Forest Critical crowd model and
            XGBoost demand model, trained on the MetroFlow dataset using a
            time-based split.
          </p>
        </Reveal>
      </section>
